...
“Configured” still does not mean that 184 autonomous services are continuously running. The repository now provides a single-node deployment with an authenticated job API, durable SQLite worker, content/configuration-addressed evidence, narrow KiCad/BOM/firmware ingestion, typed guarded SCPI PSU/DMM/scope adapters, model routing and a reference schema-validating model gateway. The built-in dry-run provider intentionally cannot approve work.

Domain reasoning becomes active only when an operator supplies a compatible external model backend and credentials. Generic drivers are unvalidated until each exact instrument model passes a conformance fixture and hardware safety test. Distributed workers, multi-user authorization, complete ECAD retrieval, physical-scene perception and representative bench validation remain work. See [`../docs/06_EXECUTABLE_CONFERENCE_RUNTIME.md`](../docs/06_EXECUTABLE_CONFERENCE_RUNTIME.md) and [`../docs/07_ENGINEERING_RUNTIME_SERVICES.md`](../docs/07_ENGINEERING_RUNTIME_SERVICES.md).
